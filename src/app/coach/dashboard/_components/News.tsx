...
function News() {
  ...
  // Check localStorage and show modal only on first visit
  useEffect(() => {
    const checkFirstVisit = () => {
      try {
        const hasVisited = localStorage.getItem('coachDashboardVisited');
        if (!hasVisited) {
          // First time visiting, show modal and set localStorage
          setIsWelcomeModalOpen(true);
          localStorage.setItem('coachDashboardVisited', 'true');
        }
      } catch (error) {
