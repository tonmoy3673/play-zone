...
const MINOR_BADGES: BadgeItem[] = [
  ...
  { id: "creative", title: "Creative", iconName: "idea" },
...
interface MajorBadgeCardProps {
  badge: MajorBadgeItem;
  isSelected: boolean;
  onBadgeToggle: (id: string) => void;
}

function BadgeCard({ badge, isSelected, onBadgeToggle }: BadgeCardProps) {
  return (
    <button
      type="button"
      onClick={() => onBadgeToggle(badge.id)}
      className={classNames(
        "flex flex-col items-center gap-4 p-5 rounded-[12px] text-center transition-all",
        "bg-[rgba(186,209,255,0.1)] border border-[#f5f6f8]",
        badge.highlight ? "border-2 border-[#5c8ff7]" : "",
        isSelected
          ? "border-primary-gradient shadow-[0_0_20px_rgba(67,126,247,0.4)]"
          : "",
        "cursor-pointer"
      )}
    >
...
function MajorBadgeCard({
  ...
  return (
    <button
      ...
      onClick={() => onBadgeToggle(badge.id)}
      className={classNames(
        "relative isolate flex w-full flex-col items-center justify-between overflow-hidden rounded-[16px] p-4 cursor-pointer text-white",
  ...
  return (
    <div className="bg-white rounded-2xl p-5 relative">
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <div
          className={`bg-[rgba(210,210,210,0.1)] rounded-[7px] px-3 items-center gap-x-3 ml-16 ${
            level === "Major" ? "flex" : "hidden"
          }`}
        >
      ...
      <div className="pointer-events-none absolute inset-x-0 bottom-0 h-32 -translate-y-20 bg-linear-to-t from-white via-[#ffffffbd] to-[#ffffff00]" />
      {/* Actions */}
