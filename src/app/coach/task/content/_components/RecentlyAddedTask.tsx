...
export default function RecentlyAddedTask() {
  ...
  return (
    <section className="bg-[rgba(255,255,255,0.3)] overflow-clip relative rounded-[24px] p-5 mt-4 border border-white">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 ">
        {programs.map((program) => (
          <div
            key={program.id}
            className="bg-white/60 rounded-2xl overflow-hidden flex flex-col h-[346px] relative"
          >
            {/* Image Section */}
            ...
            <div className="px-4 pb-4 flex flex-col gap-4 grow">
              <div className="flex flex-col gap-2 relative">
                <div className="flex items-start justify-between mt-[14px]">
                  <h3 className="text-base font-medium text-dark leading-normal w-1/2">
                    {program.title}
                  </h3>
                  <span className="bg-gradient-to-b from-[rgba(69,127,243,0.05)] to-[rgba(69,127,243,0.12)] border border-[rgba(69,127,243,0.1)] text-[#437EF7] h-6 px-2 py-2.5 rounded-full text-[10px] font-medium flex items-center justify-center whitespace-nowrap">
                    {program.tag}
                  </span>
                </div>
