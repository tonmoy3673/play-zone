...
export type CompletedCardTrend = {
  iconName: IconName;
  change: string;
  context: string;
  iconClass?: string;
...
export type CompletedCardPerson = {
  avatar: string;
  name: string;
  primary: string;
  secondary: string;
...
export type CompletedCardData = {
  title: string;
  stat?: string;
  icon: { name: IconName; wrapperClass?: string; iconClass?: string };
  trend?: CompletedCardTrend;
  person?: CompletedCardPerson;
};

type CompletedCardProps = {
  card: CompletedCardData;
};

export default function CompletedCard({ card }: CompletedCardProps) {
  return (
        ...
        {card.person ? (
          <div className="flex flex-col gap-1 items-start w-full">
            <div className="flex gap-3 items-center">
              ...
              />
              <p className="text-base font-medium text-dark-100 leading-normal">
                {card.person.name}
              </p>
            </div>
          ...
          <div className="flex flex-col gap-1 items-start w-full">
            ...
            {card.trend ? (
              <div className="flex gap-1 items-center">
                ...
                />
                <p className="text-xs font-medium text-paragraph-dark leading-[20px]">
                  <span className="text-[#38aa4b]">{card.trend.change}</span>
